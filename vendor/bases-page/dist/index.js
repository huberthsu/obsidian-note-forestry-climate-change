...
var calendar_inline_default = `var C=["January","February","March","April","May","June","July","August","September","October","November","December"];function h(e){return String(e).padStart(2,"0")}function S(e,t){return new Date(e,t+1,0).getDate()}function M(e,t){return new Date(e,t,1).getDay()}function D(e,t,n){let s=document.createElement("div");s.className="bases-calendar-day"+(t?" is-today":"");let d=document.createElement("span");if(d.className="bases-calendar-daynum",d.textContent=String(e),s.appendChild(d),n&&n.length>0){let a=document.createElement("div");a.className="bases-calendar-entries",n.forEach(c=>{let r=document.createElement("a");r.href=c.href,r.className="internal internal-link bases-calendar-chip",r.dataset.slug=c.slug,r.title=c.tooltip,r.textContent=c.title,a.appendChild(r)}),s.appendChild(a)}return s}function f(e,t,n,s,d){let a=e.querySelector(".bases-calendar-days"),c=e.querySelector(".bases-calendar-label");if(!a||!c)return;a.innerHTML="";let r=M(t,n),u=S(t,n),p=Math.ceil((r+u)/7)*7,l=document.createDocumentFragment();for(let i=0;i<p;i++){let o=i-r+1;if(o<1||o>u){let y=document.createElement("div");y.className="bases-calendar-day is-outside",l.appendChild(y);continue}let m=t+"-"+h(n+1)+"-"+h(o);l.appendChild(D(o,m===d,s[m]))}a.appendChild(l),c.textContent=C[n]+" "+t,e.dataset.currentYear=String(t),e.dataset.currentMonth=String(n),document.dispatchEvent(new CustomEvent("render"))}function w(e,t){if(e.dataset.calendarReady==="1")return;e.dataset.calendarReady="1";let n=e.querySelector('script[type="application/json"]');if(!n)return;let s;try{s=JSON.parse(n.textContent)}catch{return}let d=s.byDay||{},a=new Date,c=a.getFullYear()+"-"+h(a.getMonth()+1)+"-"+h(a.getDate());f(e,a.getFullYear(),a.getMonth(),d,c);let r=y=>{let v=Number(e.dataset.currentYear),E=Number(e.dataset.currentMonth),g=new Date(v,E+y,1);f(e,g.getFullYear(),g.getMonth(),d,c)},u=e.querySelector(".bases-calendar-prev"),p=e.querySelector(".bases-calendar-next"),l=e.querySelector(".bases-calendar-today-btn"),i=()=>r(-1),o=()=>r(1),m=()=>f(e,a.getFullYear(),a.getMonth(),d,c);u&&(u.addEventListener("click",i),t.push(()=>u.removeEventListener("click",i))),p&&(p.addEventListener("click",o),t.push(()=>p.removeEventListener("click",o))),l&&(l.addEventListener("click",m),t.push(()=>l.removeEventListener("click",m)))}function b(){let e=document.querySelectorAll(".bases-calendar");if(e.length===0)return;let t=[];e.forEach(n=>{w(n,t)}),window.addCleanup&&window.addCleanup(()=>{t.forEach(n=>{n()})})}document.addEventListener("nav",()=>{b()});document.addEventListener("render",()=>{b()});b();
`;
