...
function I(a2, u2, s3) {
  var l2 = e[i];
  e[i] = true, D = e.__b, P = e[r], $ = e.__r, U = e.unmount;
  var f2 = t(n, null);
  f2[c] = [a2];
  try {
    ...
    e[o] && e[o](a2, M), e[i] = l2, M.length = 0;
  }
}
function O(e2, t2) {
...
function R(t2, r2, o2, i2, u2, _2, b2) {
  ...
  if ("function" == typeof Z) {
      ...
      O(t2, K2), I2 = t2[a];
        ...
        for (var te = 0; y(I2) && te++ < 25; ) {
          ...
          try {
            ...
            throw _2 && e2 && "function" == typeof e2.then && (t2._suspended = true), e2;
          }
        }
        v(I2);
      }
      if (null != I2.getChildContext && (r2 = z({}, r2, I2.getChildContext())), ee && e.errorBoundaries && (Z.getDerivedStateFromError || I2.componentDidCatch)) {
        q2 = null != q2 && q2.type === n && null == q2.key && null == q2.props.tpl ? q2.props.children : q2;
        try {
          return R(q2, r2, o2, i2, t2, _2, false);
        } catch (e2) {
          return Z.getDerivedStateFromError && (I2[s] = Z.getDerivedStateFromError(e2)), I2.componentDidCatch && I2.componentDidCatch(e2, F), y(I2) ? (q2 = O(t2, r2), null != (I2 = t2[a]).getChildContext && (r2 = z({}, r2, I2.getChildContext())), R(q2 = null != q2 && q2.type === n && null == q2.key && null == q2.props.tpl ? q2.props.children : q2, r2, o2, i2, t2, _2, b2)) : H;
        } finally {
          P && P(t2), U && U(t2);
        }
      }
    ...
    try {
      ...
      return P && P(t2), e.unmount && e.unmount(t2), t2._suspended ? B(ne) : ne;
    } catch (n2) {
      if (!_2 && b2 && b2.onError) {
      ...
      return n2.then(function e2() {
        try {
          var n3 = R(q2, r2, o2, i2, t2, _2, b2);
          return t2._suspended ? B(n3) : n3;
        } catch (t3) {
...
// src/pageType.ts
import { h as h4, Fragment as Fragment5 } from "preact";
...
function createH(schema, defaultTagName, caseSensitive) {
  ...
  function h5(selector, properties, ...children) {
    let node;
...
var own2 = Object.hasOwnProperty;
...
function getFileExtension(s3) {
  return s3.match(/\.[A-Za-z0-9]+$/)?.[0];
}
...
function slugifyPath(s3) {
  return s3.split("/").map(
    (segment) => segment.replace(/\s/g, "-").replace(/&/g, "-and-").replace(/%/g, "-percent").replace(/\?/g, "").replace(/#/g, "").replace(/[<>:"|*]/g, "").toLowerCase()
  ).join("/").replace(/\/$/, "");
}
function _sluggify(s3) {
  return slugifyPath(s3);
}
function _isRelativeSegment(s3) {
  return /^\.{0,2}$/.test(s3);
}
function _addRelativeToStart(s3) {
  if (s3 === "") {
    s3 = ".";
  }
  if (!s3.startsWith(".")) {
...
var Compiler = class _Compiler {
  ...
  compileList(expression) {
    for (const element2 of expression.elements) {
      this.compileExpression(element2);
    }
  ...
  patchJump(index2, target) {
    const instruction = this.instructions[index2];
    if (!instruction) {
      throw new CompilerError("Invalid jump patch", { start: 0, end: 0 });
    }
    if (instruction.type === "Jump") {
      this.instructions[index2] = { ...instruction, offset: target - index2 };
      return;
    }
...
var Parser2 = class {
  ...
  parseIndex(object) {
    ...
    return { type: "Index", object, index: index2, span };
  }
  parseList(startSpan) {
    const elements = [];
...
function parseDuration(value) {
  ...
  let index2 = 0;
  let total = 0;
  while (index2 < trimmed.length) {
    while (trimmed[index2] === " " || trimmed[index2] === "	") {
      index2 += 1;
    }
    if (index2 >= trimmed.length) break;
    const start = index2;
    let hasDot = false;
    while (index2 < trimmed.length) {
      const ch = trimmed[index2] ?? "";
      if (isDigit2(ch)) {
        index2 += 1;
        continue;
      }
    ...
    if (start === index2) return void 0;
    const amount = Number(trimmed.slice(start, index2));
    if (Number.isNaN(amount)) return void 0;
    while (index2 < trimmed.length && (trimmed[index2] === " " || trimmed[index2] === "	")) {
      index2 += 1;
    }
    const unitStart = index2;
    while (index2 < trimmed.length && isAlpha2(trimmed[index2] ?? "")) {
      index2 += 1;
    }
    const unit = trimmed.slice(unitStart, index2).toLowerCase();
...
registerMethodFunction("string", "replace", (target, [search2, replacement]) => {
  const source = toStringValue(target);
  const needle = toStringValue(search2);
...
function getNestedValue(target, path) {
  ...
  for (const part of path) {
    ...
    if (Array.isArray(current)) {
      const index2 = Number(part);
      if (Number.isNaN(index2)) return void 0;
...
function resolveIndex(target, indexValue) {
  if (target === void 0 || target === null) return void 0;
  if (Array.isArray(target)) {
    const index2 = toNumber2(indexValue);
    if (index2 === null) return void 0;
    return target[Math.trunc(index2)];
  }
  if (typeof target === "string") {
    const index2 = toNumber2(indexValue);
    if (index2 === null) return void 0;
    return target.charAt(Math.trunc(index2));
  }
  if (isRecord2(target)) {
...
function executeLazyMethod(name, target, argPrograms, context) {
  ...
  switch (name) {
    case "filter":
      return target.filter((element2) => {
        const result = evaluateLambda(body, element2, context);
        return Boolean(result);
      });
    case "map":
      return target.map((element2) => evaluateLambda(body, element2, context));
    case "flatMap": {
      const mapped = target.map((element2) => evaluateLambda(body, element2, context));
      return mapped.flat();
    }
    case "find":
      return target.find((element2) => {
        const result = evaluateLambda(body, element2, context);
        return Boolean(result);
      });
    case "some":
      return target.some((element2) => {
        const result = evaluateLambda(body, element2, context);
        return Boolean(result);
      });
    case "every":
      return target.every((element2) => {
        const result = evaluateLambda(body, element2, context);
        return Boolean(result);
      });
    default:
...
function resolveBasesEntries(basesData, allFiles, view, selfContext) {
  ...
  for (const fd of allFiles) {
    ...
    const base2 = getBaseName(fdPath);
    if (base2 && !fileLookup.has(base2)) {
      fileLookup.set(base2, fileValue);
    }
  }
...
var g2 = globalThis;
var viewRegistry = g2[REGISTRY_KEY] ??= new ViewRegistry();
...
function ViewSelector({ views, activeIndex }) {
  ...
  return /* @__PURE__ */ jsx2("div", { class: "bases-view-tabs", role: "tablist", children: views.map((view, index2) => /* @__PURE__ */ jsx2(
    "button",
    {
      type: "button",
      class: index2 === activeIndex ? "is-active" : "",
      "data-view-index": index2,
...
function renderCellValue(value, ctx, columnId) {
  ...
  if (Array.isArray(value)) {
    const items = value.map((item, index2) => /* @__PURE__ */ jsxs2(Fragment2, { children: [
      index2 > 0 && /* @__PURE__ */ jsx4("span", { class: "bases-separator", children: ", " }),
...
var calendar_inline_default = `var C=["January","February","March","April","May","June","July","August","September","October","November","December"];function h(e){return String(e).padStart(2,"0")}function S(e,t){return new Date(e,t+1,0).getDate()}function M(e,t){return new Date(e,t,1).getDay()}function D(e,t,n){let s=document.createElement("div");s.className="bases-calendar-day"+(t?" is-today":"");let d=document.createElement("span");if(d.className="bases-calendar-daynum",d.textContent=String(e),s.appendChild(d),n&&n.length>0){let a=document.createElement("div");a.className="bases-calendar-entries",n.forEach(r=>{let c=document.createElement("a");c.href=r.href,c.className="internal internal-link bases-calendar-chip",c.dataset.slug=r.slug,c.title=r.tooltip,c.textContent=r.title,a.appendChild(c)}),s.appendChild(a)}return s}function b(e,t,n,s,d){let a=e.querySelector(".bases-calendar-days"),r=e.querySelector(".bases-calendar-label");if(!a||!r)return;a.innerHTML="";let c=M(t,n),u=S(t,n),p=Math.ceil((c+u)/7)*7,l=document.createDocumentFragment();for(let i=0;i<p;i++){let o=i-c+1;if(o<1||o>u){let y=document.createElement("div");y.className="bases-calendar-day is-outside",l.appendChild(y);continue}let m=t+"-"+h(n+1)+"-"+h(o);l.appendChild(D(o,m===d,s[m]))}a.appendChild(l),r.textContent=C[n]+" "+t,e.dataset.currentYear=String(t),e.dataset.currentMonth=String(n),document.dispatchEvent(new CustomEvent("bases-content-inserted"))}function w(e,t){if(e.dataset.calendarReady==="1")return;e.dataset.calendarReady="1";let n=e.querySelector('script[type="application/json"]');if(!n)return;let s;try{s=JSON.parse(n.textContent)}catch{return}let d=s.byDay||{},a=new Date,r=a.getFullYear()+"-"+h(a.getMonth()+1)+"-"+h(a.getDate());b(e,a.getFullYear(),a.getMonth(),d,r);let c=y=>{let v=Number(e.dataset.currentYear),E=Number(e.dataset.currentMonth),g=new Date(v,E+y,1);b(e,g.getFullYear(),g.getMonth(),d,r)},u=e.querySelector(".bases-calendar-prev"),p=e.querySelector(".bases-calendar-next"),l=e.querySelector(".bases-calendar-today-btn"),i=()=>c(-1),o=()=>c(1),m=()=>b(e,a.getFullYear(),a.getMonth(),d,r);u&&(u.addEventListener("click",i),t.push(()=>u.removeEventListener("click",i))),p&&(p.addEventListener("click",o),t.push(()=>p.removeEventListener("click",o))),l&&(l.addEventListener("click",m),t.push(()=>l.removeEventListener("click",m)))}function f(){let e=document.querySelectorAll(".bases-calendar");if(e.length===0)return;let t=[];e.forEach(n=>{w(n,t)}),window.addCleanup&&window.addCleanup(()=>{t.forEach(n=>{n()})})}document.addEventListener("nav",()=>{f()});document.addEventListener("render",()=>{f()});f();
`;
...
function parseDateParts(value) {
  ...
  if (typeof value === "string") {
    ...
    if (dateOnly) {
      const [, y2, m2, d2] = dateOnly;
      return { year: Number(y2), month: Number(m2) - 1, day: Number(d2) };
    }
...
function renderGrid(year, month0, dayInfoMap, today) {
  ...
  const cells = Array.from({ length: cellCount }, (_2, i2) => i2 - leading + 1);
  return cells.map((dayNum) => {
    ...
    return /* @__PURE__ */ jsxs4("div", { class: `bases-calendar-day${isToday ? " is-today" : ""}`, children: [
      ...
      dayEntries.length > 0 && /* @__PURE__ */ jsx6("div", { class: "bases-calendar-entries", children: dayEntries.map((e2) => /* @__PURE__ */ jsx6("a", { href: e2.href, class: "internal internal-link bases-calendar-chip", "data-slug": e2.slug, title: e2.tooltip, children: e2.title })) })
    ] });
  });
}
var CalendarView = ({
...
var CardsView = ({
  ...
  const renderCard = (entry) => {
    ...
    return /* @__PURE__ */ jsxs5("a", { href, class: "internal internal-link bases-card", "data-slug": entry.slug, children: [
      imageSrc && !isColor && /* @__PURE__ */ jsx7("div", { class: "bases-card-image", style: imageAspect, children: /* @__PURE__ */ jsx7("img", { src: imageSrc, alt: entry.title, loading: "lazy", style: { objectFit: imageFit } }) }),
  ...
  return /* @__PURE__ */ jsxs5("div", { class: "bases-cards-wrapper", children: [
    ...
    groups ? Array.from(groups.entries()).map(([label, groupEntries3]) => /* @__PURE__ */ jsxs5("div", { class: "bases-cards-group", children: [
      /* @__PURE__ */ jsxs5("div", { class: "bases-cards-group-header", children: [
        groupPropertyLabel && /* @__PURE__ */ jsxs5("span", { class: "bases-cards-group-property", children: [
          groupPropertyLabel,
          " "
        ] }),
    ...
    ] })) : /* @__PURE__ */ jsx7("div", { class: "bases-cards", style: gridStyle, children: entries.map((entry) => renderCard(entry)) })
  ] });
};
...
// src/components/views/chart.tsx
...
function hasCategory(entry, label) {
  const cats = entry.properties?.category;
  if (!Array.isArray(cats)) return false;
  return cats.some((c2) => typeof c2 === "string" && c2.includes(label));
}
...
var ChartView = ({ entries, locale, total }) => {
  ...
  const taskManagementEntry = entries.find((e2) => e2.fileProperties.basename === "Task Management");
  const props = taskManagementEntry?.properties ?? {};
  const manualAnswerKeys = toNumber3(props["\u672C\u6708\u5B8C\u6210\u8003\u53E4\u984C\u53C3\u8003\u7B54\u6848"]);
  ...
  const maxValue = Math.max(1, ...bars.map((b2) => b2.value));
  ...
  return /* @__PURE__ */ jsxs6("div", { class: "bases-chart-wrapper", children: [
    ...
    /* @__PURE__ */ jsx8(
      "svg",
      {
        ...
        children: bars.map((bar, i2) => {
          const x2 = gap + i2 * (barWidth + gap);
          const barHeight = Math.max(2, bar.value / maxValue * chartHeight);
          const y2 = topPadding + chartHeight - barHeight;
          return /* @__PURE__ */ jsxs6("g", { children: [
            ...
            /* @__PURE__ */ jsx8("rect", { x: x2, y: y2, width: barWidth, height: barHeight, fill: bar.color, rx: "4" }),
            /* @__PURE__ */ jsx8(
              "text",
              {
                x: x2 + barWidth / 2,
...
var GalleryView = ({
  ...
  const imageOpts = { slug: slug2, allSlugs, linkResolution };
  const transformOpts = { strategy: linkResolution, allSlugs };
  return /* @__PURE__ */ jsxs7("div", { class: "bases-gallery-wrapper", children: [
    /* @__PURE__ */ jsx9("div", { class: "bases-view-meta", children: formatMessage5(localeStrings.showingCount, {
      count: entries.length,
      total
    }) }),
    /* @__PURE__ */ jsx9("div", { class: "bases-gallery", style: gridStyle, children: entries.map((entry) => {
      const imageValue = imageProperty ? resolveEntryPropertyValue(imageProperty, entry) : void 0;
      const rawImage = imageValue ? String(imageValue) : "";
      const { src: imageSrc, isColor } = resolveImageSrc(rawImage, imageOpts);
      return /* @__PURE__ */ jsxs7("div", { class: "bases-gallery-item", children: [
        /* @__PURE__ */ jsx9("div", { class: "bases-gallery-image", children: imageSrc && !isColor ? /* @__PURE__ */ jsx9("img", { src: imageSrc, alt: entry.title, loading: "lazy" }) : imageSrc && isColor ? /* @__PURE__ */ jsx9("span", { class: "bases-gallery-placeholder", style: { background: imageSrc } }) : /* @__PURE__ */ jsx9(
          "span",
          {
        ...
        /* @__PURE__ */ jsx9("div", { class: "bases-gallery-title", children: /* @__PURE__ */ jsx9(
          "a",
          {
            href: transformLink(slug2, entry.slug, transformOpts),
            class: "internal internal-link",
...
function formatGroupLabel(value) {
  if (value instanceof Date && !Number.isNaN(value.getTime())) {
    ...
    const m2 = String(value.getUTCMonth() + 1).padStart(2, "0");
    const d2 = String(value.getUTCDate()).padStart(2, "0");
    return `${y2}-${m2}-${d2}`;
  }
...
var KanbanView = ({
  ...
  const orderedKeys = Array.isArray(rawColumnOrder) ? rawColumnOrder.filter((v2) => typeof v2 === "string") : [];
  const orderedGroups = [];
  ...
  for (const group of orderedGroups) {
    ...
    const cardOrder = Array.isArray(rawCardOrder) ? rawCardOrder.filter((v2) => typeof v2 === "string") : void 0;
  ...
  return /* @__PURE__ */ jsxs8("div", { class: "bases-kanban-wrapper", children: [
    /* @__PURE__ */ jsx10("div", { class: "bases-view-meta", children: formatMessage6(localeStrings.showingCount, { count: entries.length, total }) }),
    /* @__PURE__ */ jsx10("div", { class: "bases-board", children: orderedGroups.map((group) => /* @__PURE__ */ jsxs8("div", { class: "bases-board-column", children: [
      /* @__PURE__ */ jsxs8("div", { class: "bases-board-column-header", children: [
        /* @__PURE__ */ jsx10("span", { children: group.label }),
        /* @__PURE__ */ jsx10("span", { class: "bases-board-count", children: group.entries.length })
      ] }),
      /* @__PURE__ */ jsx10("div", { class: "bases-board-column-body", children: group.entries.map((entry) => {
        const ctx = { slug: slug2, allSlugs, linkResolution };
        return /* @__PURE__ */ jsxs8("div", { class: "bases-board-card", children: [
          /* @__PURE__ */ jsx10(
            "a",
            {
              href: transformLink(slug2, entry.slug, transformOpts),
          ...
          columns.length > 0 && /* @__PURE__ */ jsx10("div", { class: "bases-board-card-meta", children: columns.map((column) => {
            const value = resolveEntryPropertyValue(column, entry);
            if (isEmptyValue(value)) return null;
            return /* @__PURE__ */ jsxs8("div", { class: "bases-board-card-row", children: [
              /* @__PURE__ */ jsx10("span", { class: "bases-board-card-label", children: getColumnLabel(column, basesData) }),
              /* @__PURE__ */ jsx10("span", { class: "bases-board-card-value", children: renderCellValue(value, ctx, column) })
            ] });
          }) })
        ] });
      }) })
...
import { Fragment as Fragment3, jsx as jsx11, jsxs as jsxs9 } from "preact/jsx-runtime";
...
var ListView = ({
  ...
  const localeStrings = i18n(locale).components.bases;
  const transformOpts = { strategy: linkResolution, allSlugs };
  return /* @__PURE__ */ jsxs9("div", { class: "bases-list-wrapper", children: [
    /* @__PURE__ */ jsx11("div", { class: "bases-view-meta", children: formatMessage7(localeStrings.showingCount, {
      count: entries.length,
      total
    }) }),
    /* @__PURE__ */ jsx11("div", { class: "bases-list-group", children: /* @__PURE__ */ jsx11("div", { class: "bases-list-group-list", children: entries.map((entry) => {
      const ctx = { slug: slug2, allSlugs, linkResolution };
      const primaryColumn = columns[0] ?? "file.name";
      const secondaryColumns = columns.slice(1);
      const primaryValue = primaryColumn === "file.name" ? /* @__PURE__ */ jsx11(
        "a",
        {
          href: transformLink(slug2, entry.slug, transformOpts),
          class: "internal internal-link",
      ...
      for (const column of secondaryColumns) {
        ...
        secondaryItems.push(
          /* @__PURE__ */ jsx11("span", { class: "bases-list-property", children: /* @__PURE__ */ jsx11("span", { class: "bases-rendered-value", children: renderCellValue(value, ctx, column) }) })
        );
      }
      return /* @__PURE__ */ jsx11("div", { class: "bases-list-item", children: /* @__PURE__ */ jsxs9("div", { class: "bases-list-item-properties", children: [
        /* @__PURE__ */ jsxs9("span", { class: "bases-list-property", children: [
          /* @__PURE__ */ jsx11("span", { class: "list-bullet", children: "-" }),
          /* @__PURE__ */ jsx11("span", { class: "bases-rendered-value", children: primaryValue })
        ] }),
        secondaryItems.map((item) => /* @__PURE__ */ jsxs9(Fragment3, { children: [
          /* @__PURE__ */ jsx11("span", { class: "bases-list-separator", children: ", " }),
          item
        ] }))
      ] }) });
    }) }) })
...
import { Fragment as Fragment4, jsx as jsx12, jsxs as jsxs10 } from "preact/jsx-runtime";
function formatMessage8(template, values) {
  return Object.entries(values).reduce(
    (text, [key, value]) => text.replace(`{${key}}`, String(value)),
    template
  );
}
function groupEntries2(entries, groupProperty, emptyLabel) {
  if (!groupProperty) return null;
  const groups = /* @__PURE__ */ new Map();
  for (const entry of entries) {
    const rawValue = resolveEntryPropertyValue(groupProperty, entry);
...
function renderRow(entry, columns, view, slug2, allSlugs, linkResolution) {
  ...
  return /* @__PURE__ */ jsx12("tr", { children: columns.map((column) => {
    ...
    const style2 = columnWidth ? { width: `${columnWidth}px`, minWidth: `${columnWidth}px` } : void 0;
    return /* @__PURE__ */ jsx12("td", { "data-value": display, style: style2, children: isPrimary ? /* @__PURE__ */ jsx12(
      "a",
      {
        href: transformLink(slug2, entry.slug, transformOpts),
        class: "internal internal-link",
...
var TableView = ({
  ...
  const groupProperty = view.groupBy?.property;
  const groupPropertyLabel = groupProperty ? getColumnLabel(groupProperty, basesData) : "";
  const groups = groupEntries2(entries, groupProperty, localeStrings.uncategorized);
  return /* @__PURE__ */ jsxs10("div", { class: "bases-table-wrapper", children: [
    /* @__PURE__ */ jsx12("div", { class: "bases-view-meta", children: formatMessage8(localeStrings.showingCount, {
      count: entries.length,
      total
    }) }),
    /* @__PURE__ */ jsxs10("table", { class: "bases-table", "data-view-type": "table", children: [
      /* @__PURE__ */ jsx12("thead", { children: /* @__PURE__ */ jsx12("tr", { children: columns.map((column) => {
        const columnWidth = view.columnSize?.[column];
        const style2 = columnWidth ? { width: `${columnWidth}px`, minWidth: `${columnWidth}px` } : void 0;
        return /* @__PURE__ */ jsxs10("th", { "data-column": column, "data-sortable": "true", style: style2, children: [
          /* @__PURE__ */ jsx12("span", { class: "bases-table-header", children: getColumnLabel(column, basesData) }),
          /* @__PURE__ */ jsx12("span", { class: "bases-table-header-sort", "aria-hidden": "true" })
        ] });
      }) }) }),
      /* @__PURE__ */ jsx12("tbody", { children: groups ? Array.from(groups.entries()).map(([label, groupEntries3]) => /* @__PURE__ */ jsxs10(Fragment4, { children: [
        /* @__PURE__ */ jsx12("tr", { class: "bases-table-group-header", children: /* @__PURE__ */ jsxs10("td", { colSpan: columns.length, children: [
          groupPropertyLabel && /* @__PURE__ */ jsxs10("span", { class: "bases-table-group-property", children: [
            groupPropertyLabel,
            " "
          ] }),
      ...
      hasSummary && /* @__PURE__ */ jsx12("tfoot", { children: /* @__PURE__ */ jsx12("tr", { class: "bases-summary-row", children: columns.map((column) => {
        const summary = summaries[column];
        if (!summary) return /* @__PURE__ */ jsx12("td", {});
        const values = entries.map((entry) => resolveEntryPropertyValue(column, entry));
        return /* @__PURE__ */ jsx12("td", { children: computeSummary(values, summary) });
      }) }) })
    ] })
  ] });
};
...
function registerBuiltinViews() {
  ...
  viewRegistry.register(chartViewRegistration);
...
var BasesBody_default = ((opts) => {
  const Component = (props) => {
    ...
    const baseSlugs = new Set(rawSlugs.filter((s3) => s3.endsWith(".base")));
    const baseAliases = new Set([...baseSlugs].map((s3) => s3.replace(/\.base$/, "")));
    const allSlugs = rawSlugs.filter((s3) => !baseSlugs.has(s3) && !baseAliases.has(s3));
    const linkResolution = basesOptions?.linkResolution ?? "shortest";
    if (!basesData) {
      return /* @__PURE__ */ jsx13("div", { class: "bases-page bases-empty", children: localeStrings.noData });
    }
    const views = basesData.views ?? [];
    if (views.length === 0) {
      return /* @__PURE__ */ jsx13("div", { class: "bases-page bases-empty", children: localeStrings.noViews });
    }
    ...
    return /* @__PURE__ */ jsxs11("div", { class: "bases-page", "data-initial-view": initialIndex, children: [
      viewCssChunks.length > 0 && /* @__PURE__ */ jsx13("style", { dangerouslySetInnerHTML: { __html: viewCssChunks.join("\n") } }),
      /* @__PURE__ */ jsx13(ViewSelector, { views, activeIndex: initialIndex, locale }),
      /* @__PURE__ */ jsx13("div", { class: "bases-view-container", children: views.map((view, index2) => {
        ...
        return /* @__PURE__ */ jsx13(
          "div",
          {
            class: `bases-view ${index2 === initialIndex ? "is-active" : ""}`,
            "data-view-index": index2,
            "data-view-type": view.type,
            children: entries.length === 0 ? /* @__PURE__ */ jsx13("div", { class: "bases-empty", children: localeStrings.noData }) : Renderer ? /* @__PURE__ */ jsx13(
              Renderer,
              {
                entries,
                view,
            ...
            ) : /* @__PURE__ */ jsxs11("div", { class: "bases-empty", children: [
              "Unknown view type: ",
              view.type
            ] })
          }
...
function createBasesCodeblockTransform(opts) {
  ...
  return (root, _slug, componentData) => {
    ...
    const slug2 = componentData.fileData.slug ?? "";
...
function renderBasesInline(basesData, allFiles, locale, localeStrings, opts, slug2, allSlugs, linkResolution, viewName, selfContext) {
        ...
        () => {
          return (tree, file) => {
            ...
            visit(tree, "element", (node, index2, parent) => {
              ...
              const fullPath = join2(ctx.argv.directory, baseFile);
              let raw;
              try {
...
function buildBaseFileLookup(ctx) {
  ...
  for (const fp of baseFiles) {
    const slug2 = slugifyFilePath(fp);
    lookup.set(slug2, fp);
