---
publish: true
created: 2026-09-11
modified: 2026-09-20T12:25:44.458Z
published: 2026-09-11
category:
  - "[[English learning notes]]"
  - 單字
Chinese translation: 能力、才能、水準
字根: petere(尋求、力求)
date: 2026-09-11
---

```timestamp-url
 https://www.youtube.com/watch?v=_s1rIKaoAyM
```

```timestamp
22:58
```

## Meaning

- The ability to do something well.

---

## Word Family

- compete(v)競爭、對抗
- competently(adv)稱職地
- competent(adj)有能力的、稱職的

* incompetent(adj)無能的、不稱職的
* competitive(adj)有競爭力的、好勝的
* incompetence(n)無能、不稱職（反義詞）
* competition(n)競爭、比賽
* competitor(n)競爭對手

- Competence-Competency(n)

|         | Competence                      | Competency                   |
| ------- | ------------------------------- | ---------------------------- |
| **詞性**  | 不可數名詞為主                         | 可數名詞                         |
| **焦點**  | 「狀態」或「整體能力」                     | 「具體的能力項目」                    |
| **抽象度** | 較抽象、理論性                         | 較實務、具體                       |
| **語域**  | 學術、理論、心理學                       | 商業、HR、評估                     |
| **例句**  | _linguistic competence_ (語言學概念) | _core competencies_ (核心能力指標) |

- 字根 petere（尋求、力求，源自拉丁 petere「力求、尋求、撲向、攻擊」，PIE 字根 \*pet-「衝、飛」）（Etymonline）— competence 1590 年代最早指「競爭」（已廢用），1630 年代指「足以舒適生活的資力」，源自法語 compétence，來自拉丁 competentia「會合、一致、匀稱」，源自 competens，為 competere（尤指較早的意思「一起落下、湊在一起、合宜」）的現在分詞；competere = com「一起」+ petere；「足以應付手邊事務的能力」義 1790 年出現
  - 🤖 **claudian:** _字根 petere 是「尋求、力求」，com + petere 是「一起追求、湊在一起」，早期意思是「合宜、足夠」，所以 competence 由「合適、足以應付」引申為「能力、勝任」。_
  - compete(v) 競爭 — 晚期拉丁 competere「共同努力」，古典拉丁「湊在一起、相符、勝任」= com(一起) + petere「力求、尋求、撲向」
  - competition(n) 競爭 — 晚期拉丁 competitionem「對抗」，古典拉丁「一致」，源自 competere（見 compete）
  - competitor(n) 競爭對手 — 拉丁 competitor「對手」，源自 competere（見 compete）
  - petition(n) 請願書 — 拉丁 petitio「攻擊；尋求；訴請」，源自 petere「力求、尋求、請求」
  - appetite(n) 食慾 — 拉丁 appetitus「渴望」，源自 appetere「渴望、力求」= ad(向) + petere「去尋求」

---

## Synonyms/Related phrase

- ability
- skill
- Capability
- proficiency

> [!note] 「能力」群組比較（5詞）
> 與 ability、skill、Capability、proficiency 一起在 [[1.English words and phrases associations#「能力」群組（程度與性質差異）|1.English words and phrases associations]] 中有詳細的同主題群組比較。各詞強調層面、習得方式、應用情境皆不同——根據語境選字。

---

## Usage/Collocations

- competence+in+領域: 在某特定領域/學科上的能力

- someone be+competent+with+工具: 某人擅長使用某工具

- competence+of+someone: 某人/團體所擁有的能力

- competence+for+job/task: 有勝任某工作/職位/任務的能力

- Demonstrate / Show competence: 展現能力

- Achieve / Acquire / Gain competence: 獲得/培養能力

- Lack competence: 缺乏能力

- Core competence: 核心能力 / 核心競爭力

- Professional / Technical competence: 專業 / 技術能力

---

## Examples

- He demonstrated high competence in project management.
- All of our team members are competent with Excel.
- The success of the company depends on the competence of its employees
- The interviewer asked him to prove his competence for the job.
