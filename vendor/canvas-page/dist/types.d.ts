...
/** A `[[wikilink]]` or `![[embed]]` found inside a text node's raw text */
interface CanvasTextLink {
    /** Placeholder token substituted into the rendered HTML in place of this link */
    token: string;
    /** Raw link target as written (note name or path, not yet resolved to a slug) */
    target: string;
    /** Display text from `[[target|alias]]`, if given */
    alias?: string;
    /** Heading/block anchor (`#heading` or `#^blockid`), if given */
    subpath?: string;
    /** Whether this was a `![[...]]` embed rather than a plain `[[...]]` link */
    isEmbed: boolean;
}
...
export { CANVAS_PRESET_COLORS, type CanvasBackgroundStyle, type CanvasColor, type CanvasData, type CanvasEdge, type CanvasEnd, type CanvasFileNode, type CanvasGroupNode, type CanvasLinkNode, type CanvasNode, type CanvasPageOptions, type CanvasSide, type CanvasTextLink, type CanvasTextNode };
