...
function isTagColumn(columnId?: string): boolean {
  if (!columnId) return false;
  const normalized = columnId.replace(/^note\./, "").replace(/^file\./, "").toLowerCase();
  return normalized === "tags" || normalized === "tag";
}

function renderTagValue(value: unknown, ctx: RenderCtx): ComponentChild {
  const tag = String(value).replace(/^#/, "");
  const slugged = slugifyPath(tag);
  const href = transformLink(ctx.slug as FullSlug, `tags/${slugged}`, {
...
export function renderCellValue(value: unknown, ctx: RenderCtx, columnId?: string): ComponentChild {
